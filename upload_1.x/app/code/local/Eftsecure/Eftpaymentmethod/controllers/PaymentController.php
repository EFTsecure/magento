<?php
class Eftsecure_Eftpaymentmethod_PaymentController extends Mage_Core_Controller_Front_Action 
{
  public function gatewayAction() 
  {
    if ($this->getRequest()->get("orderId"))
    {
      ...
      Mage_Core_Controller_Varien_Action::_redirect('eftpaymentmethod/payment/response', array('_secure' => false, '_query'=> $arr_querystring));
    }
  }
  ...
  public function redirectAction() 
  {
	...
	if($orderId && $EftsecureFlag == 1){
		...
		if(strtolower($order_status) != 'pending'){
			$order->setState(Mage_Sales_Model_Order::STATE_PENDING, true, 'Payment Unsuccessfull.');
			$order->save();
		}
		
		$eftsecure_username = Mage::getStoreConfig('payment/eftpaymentmethod/eftsecure_username');
		$eftsecure_password = Mage::helper('core')->decrypt(Mage::getStoreConfig('payment/eftpaymentmethod/eftsecure_password'));
		...
		$this->loadLayout();
		$block = $this->getLayout()->createBlock('Mage_Core_Block_Template','eftpaymentmethod',array('template' => 'eftpaymentmethod/redirect.phtml'));
